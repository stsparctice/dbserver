...
async function delTableConfig(name) {
    ...
    for (let c = 0; c < Tables.length; c++) {
        if (Tables[c].MTDTable.name.name == name.tableDel) {
            if (Tables[c].MTDTable.isdesable) {//האם ניתן למחיקה
                console.log("in if");
                ...
                fs.writeFileSync('config/DBconfig.json', JSON.stringify(config));
            }
            else
                return "this table cant delete"
        }
        else {
            if (c == (Tables.length)-1&& Tables[c].MTDTable.name.name!= name.tableDel) {
